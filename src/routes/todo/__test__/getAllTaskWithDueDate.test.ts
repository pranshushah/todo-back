...
it('should get all tasks', async () => {
  ...
  await request(app)
    .post('/api/todo/new')
    .send({ todoTitle: 'my todo title', dueDate: new Date() })
    .set('Cookie', cookie);
  await request(app)
    .post('/api/todo/new')
  ...
  await request(app)
    .post('/api/todo/new')
    .send({ todoTitle: 'my todo title2', dueDate: new Date() })
    .set('Cookie', cookie);
  const res = await request(app)
    .get('/api/todo/getalltaskwithduedate')
    .set('Cookie', cookie);
...
it('should throw error because you are not logged in', async () => {
  const res = await request(app).get('/api/todo/getalltaskwithduedate');
