...
it('should get all tasks', async () => {
  ...
  await request(app)
    .post('/api/todo/new')
    .send({ todoTitle: 'my todo title', myDay: true })
    .set('Cookie', cookie);
  await request(app)
    .post('/api/todo/new')
    .send({ todoTitle: 'my todo title1', myDay: true })
    .set('Cookie', cookie);
  await request(app)
    .post('/api/todo/new')
...
it('should throw error because you are not logged in', async () => {
  const res = await request(app).get('/api/todo/getallmyday');
